# CalliOpeNLP Automatic Voice Collection Tool v1 - Brian Stasak (2026)
# Near real-time voice assessment software with interactive speech collection assistant Calli
# Participant survey; 8 different voice tasks; individual spoken instructions per recording; ASR transcripts; degree of compliance; acoustic features
# Utilized off-line Google Whisper 'tiny' ASR model/s
...
esl_voice = str(input("Is English your NATIVE first-language (No, Yes)? "))
demograph_voice = (f"ParticipantName: {name_voice}", f"ParticipantAge: {age_voice}", f"ParticipantGender: {gender_voice}", f"ParticipantHeight: {height_voice}", f"EnglishNative: {esl_voice}")
...
ao_duration = 45
fs = 44100
confidence_factor = 0.70 # degree of error allowed (dysfluencies, inserts, repeats, deletions)
model_size = "tiny" # model type: "large-v3" (runs slow); "tiny"; "small"; "base"
wordlist = []
wordlist_phrases = []
task_total_ct = []
...
def initialize_engine():
    ...
    if voices:
        engine.setProperty('voice', voices[4].id) # setup voice-type [4, 14, 17, 21] are okay
    else:
...
def extract_features(task):
    ...
    acoustic_features = (str(pitch),"AverageIntensity_dB: ", str(intensity), "Harmonicity_Stats: ", str(harmonicity))
...
def c80to90_task(task15):
    ...
    c80to90_instructions = "\tYour fourteenth task is to count from eighty to ninety continuously with no silence gaps. Press the ENTER key to start your voice recording. After you are done, please, remain quiet and the recording will stop automatically.... \n"
...
def hb_task(task16):
    ...
    hb_instructions = "\tYour fifteenth task is to sing the song Happy Birthday to any person you wish. Press the ENTER key to start your voice recording. After you are done, please, remain quiet and the recording will stop automatically.... \n"
...
def ps_task(task17):
    ...
    ps_instructions = "\tYour sixteenth task is to sing part of any popular song that you know. It can be a radio hit, commercial tune, tv theme song, hymn, or nursery rhyme. If you cannot think of a tune, you can sing, twinkle twinkle little star. Press the ENTER key to start your voice recording. After you are done, please, remain quiet and the recording will stop automatically.... \n"
...
print("\nContacting Mr. Calli, your friendly voice task assistant...")
tts_engine = initialize_engine()
...
print("\n*********************** ALL DONE WITH THE VOICE DATA COLLECTION ***********************\n")
